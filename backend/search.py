...
Two-stage search. Stage one runs in SQL and applies structured filters.
Search results can include records above the user clearance as redacted
restricted rows, while detail retrieval still enforces RBAC. Stage two decrypts
survivors and matches keywords against encrypted fields, which SQL cannot do.
"""
...
def search_records(
    ...
    """Return records matching the search list, including restricted rows.

    keyword            matches patient_id, full_name or assigned_doctor
    category           exact match on category
    max_classification optional classification ceiling for UI filters
    search_notes       also match against decrypted medical_notes
    """
    query = session.query(PatientRecord)

    # Search lists show that restricted records exist; app.record_to_dict
    # redacts their sensitive fields and get_record blocks detail access.
    ...
    if keyword and search_notes and not results:
        candidates = session.query(PatientRecord)
        if category:
